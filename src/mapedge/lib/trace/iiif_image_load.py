...
async def __fetch_img_data(session: ClientSession, tile_data, image_tiles) -> None:
    ...
    async with session.get(tile_data["url"]) as response:
        ...
        if image_data is None:
            print("💀 FAILURE ", tile_data["url"], "unsuccesfull")
            return
        # print(tile_data["url"])
        # print(start_y, end_y, start_x, end_x, image_data)
        decoded = None
        try:
        ...
        if decoded is not None:
            try:
                image_tiles[start_y:end_y, start_x:end_x] = decoded
            except ValueError:
                print(tile_data["url"])
                raise
...
async def __calculate_image_tiles(
    ...
    List[Dict[str, Union[Union[str, int], Any]]],
    str,
    Optional[int],
    Optional[int],
    Union[int, Any],
    ...
    orig_y = (
        crop_y
        if (crop_y is not None and crop_y >= 0 and crop_y < info["height"])
        else 0
    )
    orig_x = (
        crop_x if (crop_x is not None and crop_x >= 0 and crop_x < info["width"]) else 0
    )
    corr_width = crop_width if crop_width is not None else info["width"]
    end_x = (
        orig_x + corr_width if orig_x + corr_width <= info["width"] else info["width"]
    )
    corr_height = crop_height if crop_height is not None else info["height"]
    end_y = (
        orig_y + corr_height
        if orig_y + corr_height <= info["height"]
        else info["height"]
    )
    ...
    if len(format_info) > 0:  # and False:
        if "formats" in format_info[0]:
            file_type = (
                "default.tif"
                if "tif" in format_info[0]["formats"]
                else (
                    "default.png"
                    if "png" in format_info[0]["formats"]
                    else (
                        "default.jp2"
                        if "jp2" in format_info[0]["formats"]
                        else (
                            "default.webp"
                            if "webp" in format_info[0]["formats"]
                            else "default.jpg"
                        )
                    )
                )
    ...
    if (
        "tiles" in info
        and len(info["tiles"]) > 0
        and "width" in info["tiles"][0]
        and "height" in info["tiles"][0]
    ):
        # print("TILES in info.json FROM SERVER", info["tiles"])
        # TILE_SIZE = 2048
        ...
        while y < end_y:
            ...
            while x < end_x:
                ...
                # img_url = f"{url}/{int(x)},{int(y)},{int(adj_width)},{int(adj_height)}/full/0/{file_type}" ## with /full/ we run into an issue at the UU IIPsrv server, if we specify explicitly the size ourselves, we get the image as we wanted...
                img_url = f"{url}/{int(x)},{int(y)},{int(adj_width)},{int(adj_height)}/!{int(adj_width)},{int(adj_height)}/0/{file_type}"
                images.append(
                    {
                        "url": img_url,
                        "x": x - orig_x,
        ...
        res = (
            [
                {
                    "url": f"{url}/{orig_x},{orig_y},{corr_width},{corr_height}/full/0/{file_type}",
                    "x": 0,
                    "y": 0,
                    "width": corr_width,
                    "height": corr_height,
...
async def __download_manager(
    url: str, x: int = None, y: int = None, width: int = None, height: int = None
) -> ndarray:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tiles, _, _, _, corr_width, corr_height = await __calculate_image_tiles(
            session, url, x, y, width, height
        )
